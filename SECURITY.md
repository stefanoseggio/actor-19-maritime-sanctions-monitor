...
- **No credential handling of any kind.** This Actor requires no third-party API key or BYOK secret (see the README's Environment Variables section) — both source feeds (OFAC SDN.XML, UN Security Council Consolidated List) are open, unauthenticated public downloads. There is no customer secret this Actor could leak.
- **No user-supplied code execution.** Input is a fixed JSON schema (`maxItems`, `onlyNew`, `enrichWithUnConsolidatedList`, `programFilter`, `vesselNameContains`) validated against a real Zod schema (`src/schemas.ts`) — there is no arbitrary-code or arbitrary-URL input surface.
...
- **Source-feed integrity** (a compromised or spoofed OFAC/UN endpoint) is outside this Actor's control — it fetches from the feeds' own official, government/UN-published URLs over HTTPS and does not implement independent content-signing verification beyond standard TLS.
