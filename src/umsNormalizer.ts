...
const REGULATING_AGENCY = 'U.S. Department of the Treasury, Office of Foreign Assets Control (OFAC)';
const STATUS = 'Specially Designated National (Blocked)';
/**
 * Not one of the fleet's existing 11 JurisdictionCode values (AU, UK, FL,
 * AR-*, CL) - see schemas.ts's UnifiedRecordSchema comment for why 'US' is
 * the correct, non-fabricated code for a US Treasury program, and why
 * extending the shared enum is a documented follow-up, not something this
 * actor does itself.
 */
...
export function parseOfacPublishDateToIso(raw: string): string | null {
 ...
 */
export function normalizeVesselRecordToUms(raw: VesselSanctionRawRecord): UnifiedRecord {
    const candidate: UnifiedRecord = {
        record_id: raw.record_id,
        event_type: raw.event_type,
        scraped_at: raw.scraped_at,
        is_new: raw.is_new,
        source_url: raw.source_url,
        recipient_or_defendant_name: raw.vesselName,
        entity_identifier_native: raw.imoNumber,
        value_native: null,
        value_currency: null,
        value_usd_normalized: null,
        effective_date_iso: null,
        publish_date_iso: parseOfacPublishDateToIso(raw.listPublishDateRaw),
        category_or_type: raw.programs.length > 0 ? raw.programs.join('; ') : null,
        status_or_estado: STATUS,
        awarding_or_regulating_agency: REGULATING_AGENCY,
        jurisdiction: JURISDICTION,
        source_document_url: null,
        reference_number: raw.uid,
    };

    return UnifiedRecordSchema.parse(candidate);
}
 ...
 */
export function normalizeDelistedVesselToUms(raw: VesselDelistedRecord): UnifiedRecord {
    const candidate: UnifiedRecord = {
        record_id: raw.record_id,
        event_type: raw.event_type,
        scraped_at: raw.scraped_at,
        ...
        category_or_type: null,
        status_or_estado: 'Delisted (no longer present in the current OFAC SDN List)',
        awarding_or_regulating_agency: REGULATING_AGENCY,
        jurisdiction: JURISDICTION,
        source_document_url: null,
        reference_number: raw.uid,
