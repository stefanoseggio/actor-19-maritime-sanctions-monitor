 ...
 */
export async function saveState(finalEntries: Record<string, VesselEntry>, runAt: string): Promise<DeltaState> {
    ...
    const next: DeltaState = { entries: capped, lastRunAt: runAt };
    const store = await Actor.openKeyValueStore(STATE_STORE_NAME);
