...
async function run() {
    ...
    try {
        ...
        log.info(`Fetched ${records.length} vessel record(s) from the OFAC SDN List${input.enrichWithUnConsolidatedList ? ' (UN Consolidated List cross-reference enabled)' : ''}${truncatedByMaxItems ? ' [maxItems truncated the walk - DELISTED detection skipped this run]' : ''}`);
